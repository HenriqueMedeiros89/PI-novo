...
public class JogoDAO {
    ...
    public static boolean alterar(Jogo jogo) {
        try {
            Connection conexao = Conexao.getConexao();
            String sql = "UPDATE jogo SET titulo = ?, descricao = ?, preco = ?, numeroDias = ?, categoria_id = ?, memoria = ?, tipo = ?, WHERE id = ?";
            PreparedStatement ps = conexao.prepareStatement(sql);
            ps.setString(1, jogo.getTitulo());
            ps.setString(2, jogo.getDescricao());
            ps.setDouble(3, jogo.getPreco());
    ...
    public static List<Jogo> listar() {
        ...
        try {
            Connection conexao = Conexao.getConexao();
            String sql = "SELECT jogo,* categoria.nome AS categoria.tipo AS tipo_categoria FROM jogo INNER JOIN categoria ON jogo.categoria_id = categoria.id";
